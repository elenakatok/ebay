...
export function parseForm(values: FormValues, schema: OutcomeSchema = ebaySchema): ParseOk | ParseErr {
...
export default function OutcomeReporting({
  ...
  const handleConfirmDeal   = () => { const o = pendingDeal; setPendingDeal(null); withSubmit(() => submitLeadOutcome(args, o)) }
  const handleConfirmNoDeal = () => { setPendingNoDeal(false); withSubmit(() => submitLeadOutcome(args, null)) }
  ...
  if (isLead) {
      ...
      return (
        <main style={mainStyle}>
          ...
          <div style={btnRowStyle}>
            <button onClick={handleConfirmDeal} disabled={submitting}>{submitting ? 'Submitting…' : 'Yes, submit'}</button>
            <button onClick={handleCancelPending} disabled={submitting} style={ghostBtnStyle}>No, go back</button>
          </div>
        </main>
      )
    }
    ...
    if (pendingNoDeal) {
      return (
        <main style={mainStyle}>
          ...
          <p style={{ marginBottom: '1rem' }}>Submit <strong>no deal</strong> — confirm your group walked away?</p>
          {actionError && <p style={errorStyle}>{actionError}</p>}
          <div style={btnRowStyle}>
            <button onClick={handleConfirmNoDeal} disabled={submitting}>{submitting ? 'Submitting…' : 'Yes, no deal'}</button>
            <button onClick={handleCancelPending} disabled={submitting} style={ghostBtnStyle}>No, go back</button>
          </div>
        </main>
      )
    ...
    if (lead_reported_at != null) {
      return (
        <main style={mainStyle}>
          ...
          <p style={{ color: '#555' }}>{confirmedCount} of {totalCount} group member{totalCount !== 1 ? 's' : ''} confirmed.</p>
          {actionError && <p style={errorStyle}>{actionError}</p>}
        </main>
      )
    }
    ...
    return (
      <main style={mainStyle}>
        ...
        {resetCount > 0 && (
          <div style={resetBannerStyle}>A group member disagreed — coordinate and re-enter the outcome.</div>
        )}
        ...
        {actionError && <p style={errorStyle}>{actionError}</p>}
        <div style={btnRowStyle}>
          <button onClick={handleSubmitForm} disabled={submitting}>Review &amp; submit</button>
          <button onClick={handleNoDeal} disabled={submitting} style={ghostBtnStyle}>No deal</button>
        </div>
      </main>
    )
  }
  ...
  if (myConf === 'pending') {
    return (
      <main style={mainStyle}>
        ...
        {lead_outcome != null ? (
          <>
            <p style={{ marginBottom: '0.5rem', fontSize: '0.95rem', color: '#555' }}>Your lead reported:</p>
            <OutcomeCard outcome={lead_outcome} />
          </>
        ) : (
          <p style={{ fontSize: '1.05rem', marginBottom: '1rem' }}>Your lead reported: <strong>No deal</strong></p>
        )}
        <p style={{ color: '#555', marginBottom: '1.5rem' }}>Does this match what you negotiated?</p>
        {actionError && <p style={errorStyle}>{actionError}</p>}
        <div style={btnRowStyle}>
          <button onClick={handleConfirm} disabled={submitting}>{submitting ? '…' : 'Confirm'}</button>
  ...
  return (
    <main style={mainStyle}>
      ...
      <p style={{ color: '#555' }}>{confirmedCount} of {totalCount} member{totalCount !== 1 ? 's' : ''} confirmed.</p>
    </main>
  )
}
